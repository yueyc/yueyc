...
* 区域内路由：
  * Router LSA(type1):由每个路由器产生，描述路由器的链路状态和开销，在其**始发的区域内传播**。
  * Network LSA（type2）：由DR产生，描述**本网段所有路由器**的链路状态，在其始发的区域内传播。
 ...
 * OSPF划分区域之后，并非所有区域都是平等的关系，其中区域号（Area ID）是0的区域最为特殊，称之为骨干区域，骨干区域负责区域之间的路由，非主干区域之间的路由信息必须通常通过骨干区域来转发。
     * 所有非骨干区域必须与骨干区域保持连通。
          * 如果条件限制无法连通，需要在ABR（Area Border Router）之间建立虚连接（Virtual Link）予以解决。为虚连接提供一条非骨干区域内部路由的区域称为**传输区（Transit Area）**。
     * 骨干区域自身必须保持连通。
 ...
 *  只通告一条默认路由
 *  不通告任何区间路由信息
 *  不通告任何 AS-External-LSA(type5)类型路由。
